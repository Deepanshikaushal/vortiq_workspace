...
import { Search, Moon, Sun, Kanban, Table, X, Bell, User, LogIn, LogOut, Settings, Briefcase, Menu } from 'lucide-react';
...
export default function Navbar({
  ...
  return (
    <header className="glass-panel" style={{ borderRadius: '0', borderLeft: 'none', borderRight: 'none', borderTop: 'none', position: 'sticky', top: 0, zIndex: 50 }}>
      <div style={{ padding: '0.75rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
        
        {/* Mobile Header Left: Hamburger Toggle & Logo */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <div className="mobile-only" style={{ alignItems: 'center', gap: '0.5rem' }}>
            <VortiqLogo size={24} />
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: '800', fontSize: '1.05rem' }}>VortiQ</span>
          </div>
        </div>

        {/* Search Input Bar */}
        <div style={{ flex: '1', minWidth: '200px', maxWidth: '440px', position: 'relative' }}>
          ...
          <input
            ...
            style={{ width: '100%', paddingLeft: '2.5rem', paddingRight: searchQuery ? '2.3rem' : '4.5rem', height: '38px' }}
          />
          {searchQuery ? (
            <button
            ...
            <span className="desktop-only" style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', fontSize: '0.685rem', fontWeight: '700', color: 'var(--text-dim)', background: 'var(--bg-secondary)', padding: '0.15rem 0.4rem', borderRadius: '4px', border: '1px solid var(--border-color)', pointerEvents: 'none' }}>
              Ctrl K
            </span>
          )}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
          ...
          {activeWorkspace && (
            <div
              ...
              style={{
                ...
                borderRadius: '10px',
                background: 'linear-gradient(135deg, rgba(225, 29, 72, 0.22), rgba(190, 18, 60, 0.22))',
          ...
          >
            {theme === 'dark' ? <Sun size={18} style={{ color: '#fbbf24' }} /> : <Moon size={18} style={{ color: '#f43f5e' }} />}
          </button>
          ...
          {currentUser ? (
            <div style={{ position: 'relative' }} ref={menuRef}>
              ...
              >
                ...
                <span className="desktop-only" style={{ fontSize: '0.85rem', fontWeight: 700, maxWidth: '110px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
